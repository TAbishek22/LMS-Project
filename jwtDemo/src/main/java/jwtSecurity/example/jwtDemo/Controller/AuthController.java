...
public class AuthController {
    ...
    public ResponseEntity<?> register(@RequestBody MemberDisplayDTO memberDTO) {
        try {
            MemberDisplayDTO createdMember = memberClient.registerMember(memberDTO);
            return new ResponseEntity<>(createdMember, HttpStatus.CREATED);
        } catch (Exception e) { 
            return new ResponseEntity<>("Email ID already exists. Please log in.", HttpStatus.CONFLICT);
        } 
    }
}
